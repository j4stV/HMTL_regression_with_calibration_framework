...
from src.models.snn import lecun_normal_


class ProjectionHead(nn.Module):
    """2-layer MLP projection head for contrastive learning.

    Architecture: Linear → SELU → Linear → L2-normalize.
    Following SimCLR/SupCon best practices, the hidden dimension equals
    the input dimension to preserve capacity before the bottleneck.
    """
    ...
    def __init__(self, in_dim: int, proj_dim: int = 50) -> None:
        ...
        self.fc2 = nn.Linear(in_dim, proj_dim, bias=False)
        lecun_normal_(self.fc1.weight)
        lecun_normal_(self.fc2.weight)

    def forward(self, h: torch.Tensor) -> torch.Tensor:
        z = self.fc2(self.act(self.fc1(h)))
        # L2 normalize
...
class NPairsLoss(nn.Module):
    ...
    The loss is normalized by ln(batch_size) so that the output
    scale is approximately [0, 1] regardless of batch size, making
    it comparable to cross-entropy on bins.
    """
    ...
    def forward(self, projections: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            Normalized contrastive loss (approximately in [0, 1])
        """
        ...
        # Normalize by ln(batch_size) to keep loss in [0, ~1] range
        # regardless of batch size. Without this, loss grows with batch_size
        # (up to ln(4096) ≈ 8.6) and dominates the NLL regression loss.
        log_normalizer = max(math.log(max(batch_size, 2)), 1.0)
        return loss.mean() / log_normalizer


def n_pairs_loss(projections: torch.Tensor, labels: torch.Tensor, temperature: float = 0.5) -> torch.Tensor:
    """Convenience function for normalized N-pairs loss."""
